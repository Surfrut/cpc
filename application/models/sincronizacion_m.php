...
class sincronizacion_m extends CI_Model{
  ...
  function sincronizacion_modelo(){
    ...
    $query = "select ld_loc,ld_part, ld_qty_oh,ld_lot,ld_ref,ld_status, ld__chr01,cm_sort,pt_desc1,pt_desc2,pt_um,ld_date, pt_um, ld_expire from pub.ld_det inner join pub.pt_mstr on pt_part=ld_part and pt_domain='PUREFRUI' left join pub.cm_mstr on cm_addr=ld__chr01 and cm_domain='PUREFRUI' where ld_domain = 'PUREFRUI' and ld_site = 206 and pt_group like 'BULK%' and ld_qty_oh>0 WITH (NOLOCK)";
    $execute = odbc_exec($qad->conn_id, $query);

    while(odbc_fetch_array($execute)){
	  
      $query2 = "select distinct xxwo_part, xxwo_lot, xxwo_effdate, xxwo_serial from pub.xxwo_mstr where xxwo_domain='PUREFRUI' and xxwo_effdate >= '2018-01-01' and xxwo_part = '".odbc_result($execute, "ld_part")."' and xxwo_serial = '".odbc_result($execute, "ld_lot")."' and xxwo_ref = '".odbc_result($execute, "ld_ref")."' WITH (NOLOCK)";
      $execute2 = odbc_exec($qadcst->conn_id, $query2);
      // var_dump($execute2[0]." - ");
      
	  if(odbc_fetch_array($execute2)){
	  $ld_loc = odbc_result($execute, "ld_loc");
      $ld_part = odbc_result($execute, "ld_part");
      $ld_date = odbc_result($execute2, "xxwo_effdate");
